...
var wordTip = d3.tip()
    ...
    .html(function (d) {
        ...
        if (daystep-365) {
            ...
            str += d3.timeFormat('%Y')(eDatedis)  + '</h4>';
        }
...
// Get the offset position of the navbar
    ...
    d3.queue()
        .defer(d3.json,"src/data/dataWatViz.json")
        .await(ready);
...
function ready (error, dataf){
    //var wregex = new RegExp(stopWordList.join("|"),"g");
    ...
    data  = data.filter(d=> (d.time>=limit[0]&&d.time<=limit[1]));
    data.forEach(function(d) {
        d.time = parseTime(d.time);
        d.keywords = [];
        d.abstract.toLowerCase().split(" ").forEach(w=> {
            w = w.trim().replace(/\.|\,|\(|\)|\;|\:|\!|\?|\#|s$/g,'');
            if (stopWordList.find(wm=>wm==w)==undefined&& w.match(/\(\b(19|20)\d{2}\b\)|\b(19|20)\d{2}\b/g)==null&&w.length>2)
                d.keywords.push({term: w, category:"term"})})
    });
...
function handledata(data){
    ...
    if (mainconfig.IsWeekly) {
        outputFormat =  (d) => {
            return d3.timeFormat('%Y')(d3.timeSunday(d))
        };
        daystep = 365;
        svgHeight = 1000;
        mainconfig.wstep = 30;
...
function fillData(endDate, startDate) {
    ...
    while (now <= last) {
        ...
        var yy = 0;
        try {
            var presenttime = new Date(TermwDay[iterator].year);

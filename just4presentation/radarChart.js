...
function RadarChart(id, data, options, name) {
    var cfg = {
     ...
     wrapWidth: 97.5,         //The number of pixels after which a label needs to be given a new line
    ...
    /////////////////////////////////////////////////////////
        ...
        if (cfg.showText) {
            axis.append("text")
                ...
                .attr("dy", "0.35em")
                .attr("x", function (d, i) {
                    return rScale(maxValue*(1-1/cfg.levels/2) * cfg.labelFactor) * Math.cos(angleSlice[i] - Math.PI / 2);
                })
                .attr("y", function (d, i) {
                    return rScale(maxValue*(1-1/cfg.levels/2) * cfg.labelFactor) * Math.sin(angleSlice[i] - Math.PI / 2);
                })
                .text(function (d) {
                    return d
                })
    ...
    if (cfg.bin) { // bin type
        ...
        blobWrapperpath.attr("d", d => radarLine(d)).transition()
            .style("stroke-width", () => cfg.strokeWidth + "px")
            .style("stroke-opacity", d => cfg.bin ? densityscale(d.bin.val.length) : 0.1)
            .style("stroke", (d, i) => 'black')
            .style("fill", "none");
        //Create the outlines
        blobWrapper.append("path")
            .attr("class", d=>"radarStroke "+d.name+id.replace(".",""))

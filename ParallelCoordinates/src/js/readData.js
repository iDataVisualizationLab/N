...
function colorbyCategory(data,key) {
    ...
    // isolate
    listcolor= listKey.map((d)=>((d==='Virgin Islands')||(d==='Puerto Rico'))?'red':'steelBlue')
    //----
...
function object2DataPrallel_new(temp){
    ...
    temp.YearsData.filter((d,i)=> i>totalL-7).forEach((ti,it)=>{
        temp.Countries.forEach((con,i)=>{
            ...
            d3.keys( temp.YearsData[0]).forEach((ke,ki)=>{
                eachIn[temp.Variables[ki]] = ti[ke][i];
            });
            eachIn.timestep = new Date(timeslice[it]);
            eachIn.rack = con;
            eachIn.group = con;
            eachIn.name = con+', '+timeslice[it];
            eachIn.id = con+"-"+timeslice[it];
            newdata.push(eachIn);
        });
    });

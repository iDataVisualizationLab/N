let DynamicNet3D = function () {
    ...
    let graphicopt = {
        ...
        controlPanelGeneral = {
            ...
            connectType: {
                ...
                labels: ['start-end','continuous'],
                values: [0,1],
                width: '100px',
                callback: () => {
                    if (graphicopt.connectType)
                        createLineData = createLineData_c;
                    else
    ...
    function debuggerDraw(){
        if (debuugerMode){
            const g = d3.select('#debuggerForce').select('g.content');
            g.selectAll('circle').data(data.root_nodes)
                .join('circle')
                .attr('cx',d=>d.x)
                .attr('cy',d=>-d.y)
                .attr('r',3);
            debugger
            g.selectAll('line').data(solution.links)
                .join('line')
    ...
    master.init = function (arr) {
        ...
        if (debuugerMode){
            d3.select('#debuggerForce').attr('viewBox',`0 0 ${graphicopt.width*5} ${graphicopt.height*5}`)
                .select('.content').remove();
            d3.select('#debuggerForce').append('g').attr('class','content')
                .attr('transform',`translate(${graphicopt.width*5/2},${graphicopt.height*5/2})`)
        }
    ...
    function onShowLineConnectChanged() {
        const v = graphicopt.showLineConnect;
        if (v) {
            Object.values(dynamicVizs.links).forEach(l=>{
                l.el.visible = l._visible;
                if (l._visible) {
                    const {points, data, duration} = createLineData(l.el.arr);
                    l.el.data = data;
                    l.el.duration = duration;
                    const geometry = new THREE.BufferGeometry().setFromPoints(points);
                    l.el.geometry = geometry;
            ...
            //     if (d._visible) {
            //         const points = d.timeArr.map(source => new THREE.Vector3(source.x, source.y, source.z));
    ...
    function visibleNode() {
        dynamicVizs.forEach((n, ni) => {
            ...
            //     });
            data.net[ni].nodes.forEach((d, i) => {
                n.nodes.geometry.attributes.alpha.array[i] = graphicopt.showNode?n.nodes._alpha[i]:0;
            });
    ...
    function onFilterLine(){
        const value = graphicopt.filterLine-1;
        debugger
        isneedCompute=true;
        render(true);
        Object.values(dynamicVizs.links).forEach(v=>{
            v.el.duration.forEach((d,i)=>{
                if (d>=value){
                    for (let ti = v.el.data[i][0].ti;ti<=v.el.data[i][1].ti;ti++){
                        if (v.el.arr[ti])
                            dynamicVizs[ti].nodes._alpha[v.el.arr[ti]._index] = graphicopt.component.dot.opacity;
                    }
                }else{
                    for (let ti = v.el.data[i][0].ti;ti<=v.el.data[i][1].ti;ti++){
                        if (v.el.arr[ti])
                            dynamicVizs[ti].nodes._alpha[v.el.arr[ti]._index] = 0;
                    }
                }
            })
        });
    ...
    let createLineData = createLineData_c;

    function createLineData_se(path) {
        const data = [[]];
        const points = [];
        let start = 0;
        ...
        path.find((l,li)=>{
            if (l){
                data[0].push(l);
                points.push(new THREE.Vector3(l.x, l.y, l.z));
                start = l.ti;
                return l;
            }
        });

        data[0].push(path[path.length-1]);
        points.push(new THREE.Vector3(path[path.length-1].x, path[path.length-1].y, path[path.length-1].z));
        const duration = [path[path.length-1].ti-start];
        return {data, points:(duration[0]>=(graphicopt.filterLine-1))?points:[], duration};
    }
    function createLineData_c(path) {
        const data = [];
        const points = [];
        const color_ = [];
        const duration = [];
        ...
        if (duration[duration.length-1]){
            data[data.length-1][1]= path[path.length-1];
        }

        duration.forEach((d,di)=>{
            if(d>=(graphicopt.filterLine-1)){
                const el = data[di][0];const el2 = data[di][1];
                points.push(new THREE.Vector3(el.x, el.y, el.z));
                points.push(new THREE.Vector3(el2.x, el2.y, el2.z));
            }
        })
        return {data, points, duration};
    }

    function createLineSegment(path) {
        if (path.id==='iraq')
            debugger
        const {data, points,duration} = createLineData(path);
            ...
            if (dynamicVizs[0]) {
                ...
                dynamicVizs.forEach((net, ni) => {
                    net.nodes._alpha.forEach((d, i) => {
                        if (net.nodes.data.arr[i].inscreen) {
                            ...
                            temp = [temp.x, temp.y];
                            temp.index = '' + ni + ' ' + i;
                            temp.name = net.nodes.data.arr[i].name;
                            pointData.push(temp);
                        }
                    })
                });
                        ...
                        solution.forEach((sol, soli) => {
                            ...
                            if (islast) {
                                ...
                                net._links.forEach((l, li) => {
                                    let source = solution[soli].links[li].source;
                                    let target = solution[soli].links[li].target;
                                    if (typeof solution[soli].links[li].source ==="string") {
                                        source = data.net[soli].nodes[dynamicVizs.nodes[solution[soli].links[li].source].timeArr[soli]._index];
                                        target = data.net[soli].nodes[dynamicVizs.nodes[solution[soli].links[li].target].timeArr[soli]._index];
                                ...
                                if (net.deletedLinks) {
                                    net.deletedLinks.forEach((l, li) => {
                                        let source,target;
                                        if (typeof solution[soli].deletedLinks[li].source ==="string"){
                                            if (dynamicVizs.nodes[solution[soli].deletedLinks[li].source].timeArr[soli])
                                                source = dynamicVizs.nodes[solution[soli].deletedLinks[li].source].timeArr[soli];
                                            else
                                                source =dynamicVizs.nodes[solution[soli].deletedLinks[li].source].timeArr[soli-1];
                                                // source = solution[soli].deletedLinks[li].source;
                                            if (dynamicVizs.nodes[solution[soli].deletedLinks[li].target].timeArr[soli])
                                                target = dynamicVizs.nodes[solution[soli].deletedLinks[li].target].timeArr[soli];
                                            else
                                                target = dynamicVizs.nodes[solution[soli].deletedLinks[li].target].timeArr[soli-1];
                                            // source = solution[soli].deletedLinks[li].source;
                                            ...
                                            source = data.net[soli].deletedLinks[li].source;
                                            target = data.net[soli].deletedLinks[li].target;
                                        }
                                        const points = [];
    ...
    master.draw = function () {
        ...
        data.root_nodes.forEach(d=>{
            dynamicVizs.nodes[d.id]=d;
            // dynamicVizs.links[d.id] = {timeArr:d.timeArr, _visible: true};
            //         dynamicVizs.links[d.id].id = d.id;
        })
        rScale = d3.scaleSqrt().domain([d3.min(data.net,n=>d3.min(n.nodes,d=>d.freq)),d3.max(data.net,n=>d3.max(n.nodes,d=>d.freq))]).range([3,25]);
        data.net.forEach((net, ni) => {
            // time slice generate
            let sliceHolder = new THREE.Object3D();
            const netControl = {nodes: undefined, links: undefined, deletedLinks: undefined};
            netControl.nodes = createpoints(sliceHolder, net.nodes, ni, undefined);
            ...
            net.nodes.forEach(n => {
                if (!dynamicVizs.links[n.id]) {
                    dynamicVizs.links[n.id] = {timeArr:[], _visible: true};
                    dynamicVizs.links[n.id].id = n.id;
                }
                dynamicVizs.links[n.id].timeArr.push(n);
            });
        });
        if (dynamicVizs[0]) {
...
function forceFunc(data,postMessage) {
    ...
    if (net.length) {
        net.forEach(function (n, ni) {
            n.links.forEach(l=> {
                if (!linksObjs[l.source + l.target])
                    ...
                    linksObjs[l.source + l.target].value+= (l.value??1);
                    if (linksObjs[l.source + l.target].value>strengthRange[1])
                        strengthRange[1] = linksObjs[l.source + l.target].value
                }
            })
        });

        // linkstrength.domain(strengthRange);
        ...
        const force = d3.forceSimulation()
            ...
            // .force("link", d3.forceLink().id(d => d.id))//.strength(l=>linkstrength(l.value)))
            .force("link", d3.forceLink().id(d => d.id).distance(d=>10).strength(0.9).iterations(3))//.strength(l=>linkstrength(l.value)))
            .on('tick',function(){
                minAlpha = force.alpha();
            }).on('end',function(){
                data.net.links= links_arr
                postMessage({
                    action: 'stable',
        ...
        const links_arr = Object.values(linksObjs);
        console.log('#nodes ',nodes.length);
        console.log('#nodes ',links_arr.length);
        force.nodes(nodes);
        force.force("link").links(links_arr);
    } else {
        postMessage({
            action: 'stable',

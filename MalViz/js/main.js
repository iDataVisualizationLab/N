function applicationManager(globalData) {
    ...
    function DataRetrieval(inputData) {
        ...
        // Ngan 2/29
        group_by_process_name.forEach(d=>{
            d.timeRange = d3.extent(d.values,e=>new Date(e.currenttimestamp));
            d.stepRange = d3.extent(d.values,e=>new Date(e.Step));
        });
    ...
    return {
        ...
        drawMain: function (position) {
            ...
            var rect_height = 20, rect_margin_top = 5, group_rect_height = rect_height;
...
// MOUSEMOVE =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
            ...
            group_by_process_name.forEach(function (row, index) {
                ...
                group.append('text')
                    ...
                    .attr('x', () => {
                        return StepScale(row.stepRange[1]) * rect_width + margin_left + 5
                    })
                    .attr('y', group_rect_height / 2)
                    .attr('text-anchor', 'start')

    ...
    d3.queue()
        ...
        .await((error, data)=>{
            ...
            data.forEach(d=>{
                var time;
                if (d.source == "reuters")
                    time = outputFormat(d.time);
                else
                    time = outputFormat(parseTime(d.time));
                out+="\n"+d.source+"\t";
                out+= time + "\t";
                var per =[];
                var loc =[];
                ...
                d.keywords.forEach(k=>{
                    switch (k.category) {
                        ...
                        case 'PRODUCT':
                            ...
                            break;
                        case 'ORG':
                            if (org.find(kk=>kk==k.term) === undefined && blackw.find(kk=>kk==k.term) === undefined) {
                                org.push(k.term.replace(/\"/g,""));
                                orgs+=k.term.replace(/\"/g,"")+"|";
                            }
                            break;
                        case 'MONEY':
                        case 'PERCENT':
                            if (mis.find(kk=>kk==k.term) === undefined && blackw.find(kk=>kk==k.term) === undefined) {
                                mis.push(k.term.replace(/\"/g,""));
                                miss+=k.term.replace(/\"/g,"")+"|";
                            }
                            break;
